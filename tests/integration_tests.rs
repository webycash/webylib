...
async fn test_full_wallet_integration() {
    ...
    let wallet = Wallet::open("integration_test_wallet.db").await
        .expect("Failed to create wallet");

    let webcash = SecretWebcash::parse(&test_secret)
        .expect("Failed to parse TEST_WEBCASH_SECRET");

    wallet.insert(webcash.clone()).await
    ...
    println!("📊 Wallet stats: {} total, {} unspent", stats.total_webcash, stats.unspent_webcash);
...
async fn test_server_api_endpoints() {
    ...
    let server_client = crate::server::ServerClient::new()
        .expect("Failed to create server client");

    let webcash = SecretWebcash::parse(&test_secret)
        .expect("Failed to parse test webcash");
    let public_webcash = vec![webcash.to_public()];

    match server_client.health_check(&public_webcash).await {
        Ok(response) => println!("✅ Health check successful: {} entries", response.results.len()),
        Err(e) => println!("⚠️  Health check failed: {}", e),
    }

    match server_client.get_target().await {
        Ok(target) => println!("✅ Target query successful: difficulty {}", target.difficulty_target_bits),
...
async fn test_wallet_operations_offline() {
    ...
    let wallet = Wallet::open("offline_test_wallet.db").await
    ...
    wallet.store_master_secret(test_master_secret).await.unwrap();

    let test_webcash = SecretWebcash::parse("e1.00000000:secret:abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890")
        .unwrap();
    wallet.insert_with_validation(test_webcash.clone(), false).await.unwrap();
...
async fn test_cross_wallet_hd_recovery_integration() {
    ...
    let primary_wallet = Wallet::open("primary_test_wallet.db").await
        .expect("Failed to create primary wallet");

    primary_wallet.store_master_secret(master_secret).await
    ...
    match primary_wallet.insert(webcash.clone()).await {
        ...
        Err(e) => {
            if e.to_string().contains("spent") {
                primary_wallet.insert_with_validation(webcash.clone(), false).await
                    .expect("Failed to insert test webcash");
            } else {
    ...
    let pay_secret_hex = hd_wallet.derive_secret(KeyType::Pay.to_encoding() as u64, 0)
        .expect("Failed to derive payment secret");
    ...
    let secondary_wallet = Wallet::open("secondary_test_wallet.db").await
    ...
    secondary_wallet.insert(payment_webcash_for_insert.clone()).await
        .expect("Failed to insert payment");

    let _recovery_result = secondary_wallet.recover(master_secret, 5).await
...
async fn test_cli_manual_workflow() {
    ...
    let input_webcash = webylib::SecretWebcash::parse(&test_secret)
        .expect("❌ Failed to parse input webcash");
    ...
    if !PathBuf::from(cli_binary).exists() {
        panic!("❌ CLI binary not found at {}. Run 'cargo build --release' first", cli_binary);
    }
    ...
    let master_secret_obj = CryptoSecret::generate()
        .expect("❌ Failed to generate master secret");
    let master_secret = master_secret_obj.to_hex();
    
    fs::write(master_secret_path, &master_secret)
    ...
    let final_pay_output = Command::new(cli_binary)
        .args(&["--wallet", wallet_path, "pay", &amount_str, "-m", "Final output"])
        .output()
            ...
            if line.contains("Send this webcash to recipient:") {
                if let Some(recipient_pos) = line.find("recipient:") {
                    ...
                    if let Some(e_pos) = after_recipient.find("e") {
                        ...
                        let end = candidate_str.find(|c: char| c.is_whitespace() || c == '\n' || c == '\r')
    ...
    let output_webcash = webylib::SecretWebcash::parse(&output_webcash_str)
        .expect("❌ Invalid output webcash");
    ...
    assert!(output_webcash.amount > Amount::ZERO,
...
async fn recover_and_pay(
    ...
    match wallet.recover(master_secret, 20).await {
        Ok(summary) => {
            ...
            let balance = wallet.balance().await.expect("❌ Failed to get balance after recovery");
            let balance_amount = Amount::from_str(&balance).expect("❌ Failed to parse balance");
            
            if balance_amount >= target_amount {
                match wallet.pay(target_amount, "Final output after recovery").await {
                    Ok(msg) => extract_webcash_from_message(&msg, target_amount),
                    Err(_e) => {
                        let remaining = wallet.list_webcash().await.expect("❌ Failed to list after recovery");
        ...
        Err(e) => {
            ...
            let remaining = wallet.list_webcash().await.expect("❌ Failed to list webcash");
            if let Some(first) = remaining.first() {
                first.to_string()
            } else {
...
async fn test_complete_money_preservation() {
    ...
    let input_webcash = SecretWebcash::parse(&test_secret)
        .expect("❌ Failed to parse input webcash");
    let input_amount = input_webcash.amount;
    ...
    let wallet = Wallet::open(wallet_path).await
        .expect("❌ Failed to create wallet");
    ...
    wallet.store_master_secret(&master_secret).await
        .expect("❌ Failed to store master secret");
    ...
    let payment_amount = Amount::from_str("0.00001")
        .expect("❌ Failed to parse payment amount");
    
    let payment_result = wallet.pay(payment_amount, "Test payment").await;
    let payment_webcash_str = match payment_result {
        Ok(msg) => {
            extract_webcash_from_message(&msg, payment_amount)
        }
        Err(e) => {
    ...
    if !payment_webcash_str.is_empty() {
        let payment_webcash = SecretWebcash::parse(&payment_webcash_str)
            .expect("❌ Failed to parse payment webcash");
    ...
    let final_balance_amount = Amount::from_str(&final_balance)
    ...
    let output_webcash = SecretWebcash::parse(&output_webcash_str)
        .expect("❌ Invalid output webcash");
...
async fn test_online_amount_preservation() {
    ...
    let test_master_secret = "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
    wallet.store_master_secret(test_master_secret).await
        .expect("❌ Failed to store master secret");
...
async fn test_phase2_all_operations() {
    ...
    match insert_result {
        ...
        Err(e) => {
            if e.to_string().contains("spent") {
                wallet.insert_with_validation(webcash.clone(), false).await
    ...
    let webcash_list = wallet.list_webcash().await.expect("❌ Failed to list webcash");
    ...
    let public_webcash: Vec<PublicWebcash> = webcash_list.iter()
        .map(|wc| wc.to_public())
        .collect();
    ...
    let master_secret_bytes = hex::decode(test_master_secret)
        .expect("❌ Failed to decode master secret");
    let mut master_secret_array = [0u8; 32];
    master_secret_array.copy_from_slice(&master_secret_bytes);
    
    let mut hd_wallet = HDWallet::from_master_secret(master_secret_array);
    
    let _receive_secret = hd_wallet.generate_receive_secret(0)
        .expect("❌ Failed to generate receive secret");
    
    let _pay_secret = hd_wallet.generate_pay_secret(0)
        .expect("❌ Failed to generate pay secret");

    let recovery_wallet_path = "phase2_recovery_wallet.db";
    let _ = fs::remove_file(recovery_wallet_path);
    
    let recovery_wallet = Wallet::open(recovery_wallet_path).await
        .expect("❌ Failed to create recovery wallet");
    
    recovery_wallet.store_master_secret(test_master_secret).await
        .expect("❌ Failed to store master secret");
...
async fn create_biometric_wallet(temp_dir: &TempDir, wallet_name: &str) -> (Wallet, PathBuf) {
    let wallet_path = temp_dir.path().join(format!("{}_biometric.db", wallet_name));
    let wallet = Wallet::open_with_biometric(&wallet_path, true).await
        .expect("Should create biometric wallet");
    
    (wallet, wallet_path)
}

async fn populate_wallet_with_test_data(wallet: &Wallet) -> std::result::Result<(), Box<dyn std::error::Error>> {
...
async fn test_wallet_encryption_basic_workflow() {
    ...
    let initial_balance = wallet.balance().await
        .expect("Should get initial balance");
    
    let password = "test_encryption_password";
    let encrypted_data = wallet.encrypt_with_password(password).await
    ...
    new_wallet.decrypt_with_password(&encrypted_data, password).await
        .expect("Should decrypt wallet");
    
    let restored_balance = new_wallet.balance().await
        .expect("Should get restored balance");
    
    assert_eq!(initial_balance, restored_balance, "Balance should be preserved");
}
...
async fn test_biometric_wallet_creation() {
    ...
    assert!(wallet.is_biometric_enabled(), "Biometric encryption should be enabled");
    
    let available = wallet.is_biometric_available().await;
    assert!(available.is_ok(), "Biometric availability check should not error");
...
async fn test_biometric_wallet_encryption() {
    ...
    new_wallet.decrypt_with_biometrics(&encrypted_data).await
...
async fn test_wallet_encryption_large_dataset() {
    ...
    let initial_stats = wallet.stats().await
    ...
    let restored_stats = new_wallet.stats().await
        .expect("Should get restored stats");
    
    assert_eq!(initial_stats.unspent_webcash, restored_stats.unspent_webcash, "Webcash count should be preserved");
}
...
async fn test_cross_wallet_migration() {
    ...
    populate_wallet_with_test_data(&source_wallet).await
        .expect("Should populate source wallet");
    
    let (dest_wallet, _dest_path) = create_biometric_wallet(&temp_dir, "destination").await;
    
    let source_balance = source_wallet.balance().await
        .expect("Should get source balance");
    
    let password = "migration_password";
    let encrypted_data = source_wallet.encrypt_with_password(password).await
        .expect("Should encrypt source wallet");
    
    dest_wallet.decrypt_with_password(&encrypted_data, password).await
        .expect("Should decrypt to destination wallet");
    
    let dest_balance = dest_wallet.balance().await
        .expect("Should get destination balance");
    
    assert_eq!(source_balance, dest_balance, "Migration should preserve balance");
    assert!(dest_wallet.is_biometric_enabled(), "Destination should have biometric enabled");
}
...
async fn test_encryption_error_handling() {
    ...
    populate_wallet_with_test_data(&wallet).await
        .expect("Should populate wallet");
    
    let password = "test_password";
    let mut encrypted = wallet.encrypt_with_password(password).await
        .expect("Should encrypt wallet");
    ...
    let corrupt_result = corrupt_wallet.decrypt_with_password(&encrypted, password).await;
    assert!(corrupt_result.is_err(), "Should error on invalid algorithm");
    
    let mut encrypted = wallet.encrypt_with_password(password).await
        .expect("Should encrypt wallet");
    encrypted.ciphertext = "invalid_hex".to_string();
    
    let (invalid_wallet, _invalid_path) = create_test_wallet(&temp_dir, "invalid_data").await;
    let invalid_result = invalid_wallet.decrypt_with_password(&encrypted, password).await;
...
async fn test_wallet_stats_preservation() {
    ...
    let original_stats = wallet.stats().await
        .expect("Should get original stats");
    
    let password = "stats_preservation_test";
    let encrypted = wallet.encrypt_with_password(password).await
        .expect("Should encrypt wallet");
    
    let (restored_wallet, _restored_path) = create_test_wallet(&temp_dir, "stats_restored").await;
    restored_wallet.decrypt_with_password(&encrypted, password).await
        .expect("Should restore wallet");
    
    let restored_stats = restored_wallet.stats().await
        .expect("Should get restored stats");
    
    assert_eq!(original_stats.total_webcash, restored_stats.total_webcash, "Total webcash count should match");
    assert_eq!(original_stats.unspent_webcash, restored_stats.unspent_webcash, "Unspent webcash count should match");
}
...
async fn test_encryption_performance() {
    ...
    let (restore_wallet, _restore_path) = create_test_wallet(&temp_dir, "performance_restored").await;
    
    let decrypt_start = std::time::Instant::now();
    restore_wallet.decrypt_with_password(&encrypted, password).await
        .expect("Should decrypt wallet");
    ...
    assert!(encrypt_duration.as_secs() < 5, "Encryption should complete within 5 seconds");
    assert!(decrypt_duration.as_secs() < 5, "Decryption should complete within 5 seconds");
}
...
async fn test_concurrent_wallet_operations() {
    ...
    for i in 0..5 {
        ...
        let handle = task::spawn(async move {
            ...
            let (restore_wallet, _restore_path) = create_test_wallet(&temp_dir, &format!("concurrent_restore_{}", i)).await;
            restore_wallet.decrypt_with_password(&encrypted, &password).await
                .expect("Concurrent decryption should succeed");
            
            let balance = restore_wallet.balance().await
                .expect("Should get concurrent balance");
            
            assert!(!balance.is_empty(), "Concurrent balance should not be empty");
        });
...
async fn test_runtime_database_encryption_workflow() {
    ...
    let test_webcash = SecretWebcash::parse("e0.00001:secret:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef")
        .expect("Failed to create test webcash");

    wallet.store_directly(test_webcash).await
        .expect("Failed to store test webcash");

    let initial_balance = wallet.balance_amount().await
    ...
    let file_contents = fs::read(&wallet_path)
    ...
    let wallet2 = Wallet::open_with_biometric(&wallet_path, true).await
        .expect("Failed to open encrypted wallet");

    let restored_balance = wallet2.balance_amount().await
        .expect("Failed to get balance from decrypted wallet");
    
    assert_eq!(initial_balance, restored_balance);

    let webcash_list = wallet2.list_webcash().await
...
async fn test_regular_wallet_unaffected() {
    ...
    let test_webcash = SecretWebcash::parse("e0.00001:secret:1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")
        .expect("Failed to create test webcash");

    wallet.store_directly(test_webcash).await
        .expect("Failed to store test webcash");

    let balance = wallet.balance_amount().await
    ...
    let file_contents = fs::read(&wallet_path)
        .expect("Failed to read regular database");
    assert!(file_contents.starts_with(b"SQLite format 3"));

    let wallet2 = Wallet::open(&wallet_path).await
...
async fn test_encrypted_wallet_requires_biometric_flag() {
...

    {
        ...
        let test_webcash = SecretWebcash::parse("e0.00001:secret:abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890")
            .expect("Failed to create test webcash");
...
async fn test_multiple_encryption_cycles() {
    ...
    let initial_webcash = SecretWebcash::parse("e0.00001:secret:c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7c7")
        .expect("Failed to create test webcash");

    for cycle in 1..=3 {
        let wallet = if cycle == 1 {
            let w = Wallet::open_with_biometric(&wallet_path, true).await
                .expect("Failed to create wallet");
            w.store_directly(initial_webcash.clone()).await
                .expect("Failed to store initial webcash");
            w
        } else {
            Wallet::open_with_biometric(&wallet_path, true).await
        ...
        let cycle_webcash = SecretWebcash::parse(
            &format!("e0.0000{}:secret:{:064x}", cycle, cycle as u64 * 0x1111111111111111u64)
        ).expect("Failed to create cycle webcash");
        
        wallet.store_directly(cycle_webcash).await
    ...
    let final_wallet = Wallet::open_with_biometric(&wallet_path, true).await
        .expect("Failed to open wallet for final verification");
    
    let unspent = final_wallet.list_webcash().await
